...
void Gestor_Func::GenerateMap()
{
	...
	while (p != lista.end())
	{
		if (map_of_salarios.insert(make_pair((*p)->GetNum(),(*p)->Calc_Ordenado())).second == false)
		{
			cout << "\nFuncionario com o numero " << (*p)->GetNum() << " nao foi inserido na tabela de vencimentos porque ja existe!";
		}
		p++;
	}
